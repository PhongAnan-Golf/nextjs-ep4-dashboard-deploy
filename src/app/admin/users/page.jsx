...
function AdminUserManagePage() {
  ...
  const getAllUsersData = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/totalusers`, { cache: 'no-store' });
                  ...
                  <tbody>
                    {allUsersData?.map((val) => (
                      <tr key={val._id}>
                        ...
                        <td className='p-5'>
                          <Link className='bg-gray-500 text-white border py-2 px-3 rounded texy-ls my-2' href={`/admin/users/edit/${val._id}`}>
                            Edit
                          </Link>
